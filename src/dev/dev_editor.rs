...
pub(super) fn plugin(app: &mut App) {
    ...
    .add_systems(Update,
        (handle_debug_render, set_cursor_grab_mode)
...
impl EditorWindow for DevEditorWindow {
    ...
    const NAME: &'static str = "dark_forest Dev";
    const DEFAULT_SIZE: (f32, f32) = (200., 150.);
    fn ui(
        _world: &mut World,
        mut cx:
        bevy_editor_pls::editor_window::EditorWindowContext,
        ui: &mut egui::Ui,
    ) {
        let state = cx.state_mut::<DevEditorState>()
...
fn set_cursor_grab_mode(
    ...
    for event in events.read() {
        if let EditorEvent::Toggle { now_active } = event {
            if *now_active {
                force_cursor_grab.0 = Some(CurosrGrabMode::None);
            } else {
